...
def confirm_page():
    ...
    if check == 0:
        ...
        val = (data[0], request.form['apply'], "WAITING")
        mycursor.execute(sql, val)  
        mydb.commit()
...
def job_page():
    ...
    if request.method == "GET":
        ...
        mycursor = mydb.cursor()
        sql = "INSERT INTO announcements(title, company_id, applicants_number, initial_date, deadline, working_day, salary) VALUES (%s, %s,%s, %s,%s, %s,%s)"
        val = (form_title,data[0] , 0, date.today(), deadline, working_day, salary)
        mycursor.execute(sql, val)
        mydb.commit()
